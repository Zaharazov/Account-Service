...
func AddEvent(user_id, event_id uuid.UUID) error {
	organizer, err := GetOrganizers(user_id, 1)
	if err != nil {
		return err
	}
	events_list := organizer[0].CreatedEvents
	events_list = append(events_list, event_id)

	filter := bson.D{{"userid", user_id}}

	update := bson.D{
		{"$set", bson.D{
			{"createdevents", events_list},
		}},
	}
